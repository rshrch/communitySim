...
const mergeTargetSecret = process.env.MERGE_TARGET_SECRET || null;
...
async function mergeAccountsInto(targetSecret) {
  const mergeTargetKeypair = Keypair.fromSecret(targetSecret);
  const mergeTargetPub = mergeTargetKeypair.publicKey();

  console.log(`🔄 Merging all confirmed accounts into: ${mergeTargetPub}`);
  let merged = 0, failed = 0;

  for (const r of results) {
    if (!r.confirmed || !r.seed) continue;

    try {
      const sourceKeypair = Keypair.fromSecret(r.seed);
      const acct = await horizonTest.loadAccount(sourceKeypair.publicKey());

      const tx = new TransactionBuilder(acct, {
        fee: BASE_FEE.toString(),
        networkPassphrase: Networks.TESTNET,
      })
        .addOperation(Operation.accountMerge({ destination: mergeTargetPub }))
        .setTimeout(30)
        .build();

      tx.sign(sourceKeypair);

      await horizonTest.submitTransaction(tx);
      merged++;
    } catch (e) {
      failed++;
      console.warn(`❌ Merge failed for ${r.pub}:`, e?.response?.data?.extras || e.message);
    }
  }

  console.log(`✅ Merge complete: ${merged} merged, ${failed} failed.`);
}
...
(async () => {
  ...
  if (mergeTargetSecret) {
    await mergeAccountsInto(mergeTargetSecret);
  } else {
    console.warn('⚠️ No MERGE_TARGET_SECRET provided. Merge step skipped.');
  }
